# Stories 018–020: 사진별 티어 실제 가동 (암컷 사진 → 출제 런타임 → 승급 알림)

**작성:** 2026-09-25 · **상태:** Draft (사용자 검토 대기) · **순번 조정:** 파티 세션 결정으로 STORY-018 '오늘 만날 새'가 먼저. 017→018, 018→019, 019→020  
...
**Dependencies:** STORY-018 (암컷 사진 없이는 동작 확인 불가. 단, 테스트는 합성 데이터로 가능해 병렬 착수는 가능)
...
**Dependencies:** 없음 (STORY-018/019과 독립. 순서상 마지막에 두는 이유는 019로 화면이 바뀐 뒤 배너 위치를 잡는 게 낫기 때문)
...
| 순서 | 스토리 | PR | 사용자 개입 |
|---|---|---|---|
| 1 | STORY-018 암컷 사진 수집 | `photo-female-ducks` | 콘택트시트 14장 검수·선택 |
| 2 | STORY-019 출제 사진 런타임 | `quiz-photo-tiers` | Firefox에서 Lv3 세션 한 번 확인(진도 초기화 후 테스트용 레벨 강제는 `?level=3` 쿼리로 — 개발 전용, 프로덕션 빌드에서는 무시) |
| 3 | STORY-020 승급 알림 | `level-up-banner` | 없음 |
...
2. STORY-019의 개발용 `?level=3` 쿼리를 둘지, 아니면 진도 저장소를 직접 조작해 테스트할지. → 쿼리가 QA에 편하므로 두되 프로덕션 빌드에서 무시하는 것을 제안.
